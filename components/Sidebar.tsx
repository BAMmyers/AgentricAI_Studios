...
import type { DynamicNodeConfig, ExecutionHistoryEntry, SavedWorkflow } from '../src/core/types';

type PanelType = 'nodes' | 'history' | 'workflows';
...
const SidebarButton: React.FC<{ icon: string; label: string; isActive: boolean; onClick: () => void }> = ({ icon, label, isActive, onClick }) => (
  <button
    ...
    title={label}
  >
    <span className="text-sm font-bold tracking-tighter">{icon}</span>
  </button>
);
