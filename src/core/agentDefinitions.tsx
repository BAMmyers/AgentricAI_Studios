...
export const initialSystemAgents: DynamicNodeConfig[] = [
  ...
  },
  {
    ...
    description: "Transforms input data from one format to another based on a user-provided transformation logic.",
    icon: "🪄",
    color: "bg-indigo-600",
    category: "Utility",
    inputs: [
        { id: "input_data_in", name: "Input Data", dataType: "any" },
        { id: "transformation_logic_in", name: "Transformation Logic", dataType: "text" },
    ],
    outputs: [ { id: "transformed_data_out", name: "Transformed Data", dataType: "any" } ],
    executionLogicPrompt: `You are a data transformation engine. Take the input data: {input_data_in}. Apply the following transformation logic to it: '{transformation_logic_in}'. The final output must be a single JSON object with the key 'transformed_data_out' containing the result. For example: {"transformed_data_out": <your_result>}.`,
    isJuggernaut: true,
    isDynamic: true,
  },
  {
  ...
  {
    name: "Web Scraper",
    description: "Extracts specified information from a given URL. Note: This requires the Gemini runtime with web search enabled.",
    icon: "🕸️",
    color: "bg-green-700",
    category: "Research",
    inputs: [
        { id: "url_in", name: "URL", dataType: "text" },
        { id: "extraction_query_in", name: "Extraction Query", dataType: "text" },
    ],
    outputs: [ { id: "scraped_data_out", name: "Scraped Data", dataType: "text" } ],
    executionLogicPrompt: `You are a web scraping assistant. Access the content of the URL provided: {url_in}. From the content of that page, extract the information described in the following query: '{extraction_query_in}'. Present the extracted information as a clean text output.`,
    isJuggernaut: true,
    isDynamic: true,
    requiresWebSearch: true,
  },
  ...
  {
    name: "Code Generator",
    description: "Writes code in a specified language based on a natural language description of the task.",
    icon: "💻",
    color: "bg-blue-700",
    category: "Code / Execution",
    inputs: [
        { id: "task_description_in", name: "Task Description", dataType: "text" },
        { id: "language_in", name: "Language", dataType: "text" },
    ],
    outputs: [ { id: "generated_code_out", name: "Generated Code", dataType: "text" } ],
    executionLogicPrompt: `You are an expert code generator. Based on the task description '{task_description_in}', write a clean, well-commented script in the language '{language_in}' to accomplish the task. Provide only the code, wrapped in a markdown block appropriate for the language.`,
    isJuggernaut: true,
    isDynamic: true,
  },
];
